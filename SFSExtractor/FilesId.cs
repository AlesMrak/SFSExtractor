...
{
    using Editor.Configuration.WW2Project;
    ...
    public sealed class FilesID : IEnumerable
    {
        ...
        public static bool IsMapDir(string name)
        {
            ...
            return SFSStream.Exists(Path.Combine(WW2Options.Instance.Path.WW2_DATA, text));
        }
        ...
        public static bool IsSoundAmbientPreset(string name)
        {
            ...
            {
                return SFSStream.Exists(Path.Combine(WW2Options.Instance.Path.WW2_DATA, name));
            }
            return false;
        }
        ...
        {
            this.Clear();
            using (SFSReader reader = new SFSReader(Path.Combine(WW2Options.Instance.Path.WW2_DATA, "data/settings/filesid.ini")))
            {
